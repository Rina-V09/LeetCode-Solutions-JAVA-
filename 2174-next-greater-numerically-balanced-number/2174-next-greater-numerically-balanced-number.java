...
class Solution {
    ...
    // Precomputed list of all balanced (beautiful) numbers in sorted order
    private static final int[] balancedNumbers = new int[] {
    ...
    public int nextBeautifulNumber(int currentNumber) {
        // Step 1: Use binary search to find where currentNumber + 1 fits
        int index = Arrays.binarySearch(balancedNumbers, currentNumber + 1);

        // Step 2: If not found, fix the negative insertion point
        if (index < 0) {
            index = -index - 1; // This gives the index of the next greater number
        }

        // Step 3: Return that next beautiful number
